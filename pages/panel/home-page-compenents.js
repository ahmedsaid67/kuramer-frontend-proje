...
export default function Sayfalar() {
    ...
    const getData = async () => {
      ...
      setHasError(false);
      try {
        const response = await axios.get(API_ROUTES.HOME_PAGE_COMPENENTS_PAGINATIONS.replace("currentPage",currentPage))
    ...
    const handlePageChange = (event, value) => {
      ...
      const handleSave = (editedItem) => {
        ...
        if (!editedItem.img || !editedItem.icerik || !editedItem.baslik || !editedItem.alt_baslik  ) {
        ...
        formData.append("alt_baslik", editedItem["alt_baslik"]);
        ...
        setIsSaving(true);
        axios.put(API_ROUTES.HOME_PAGE_COMPENENTS_DETAIL.replace("id",editedItem.id), formData)
          .then(response => {
            // Mevcut sayfayı yeniden yüklüyoru
            return axios.get(API_ROUTES.HOME_PAGE_COMPENENTS_PAGINATIONS.replace("currentPage",currentPage))
          })
          .then(response => {
    ...
    return(
                  ...
                  <Table size="small">
                    <TableHead>
                      <TableRow style={{ backgroundColor: '#1976d2' }}> 
                        ...
                        <TableCell style={{ fontSize: '0.85rem', fontWeight: 'bold', color: '#fff' }}>Başlık</TableCell>
                        <TableCell style={{ fontSize: '0.85rem', fontWeight: 'bold', color: '#fff' }}>Alt Başlık</TableCell>
                        ...
                        <TableRow key={row.id}>
                          ...
                          <TableCell style={{ fontSize: '0.75rem', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            <Tooltip title={row.baslik} placement="top">
                              <span>{truncateText(row.baslik, 30)}</span>
                            </Tooltip>
                          </TableCell>
                          <TableCell style={{ fontSize: '0.75rem', maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                            <Tooltip title={row.alt_baslik} placement="top">
                              <span>{truncateText(row.alt_baslik, 30)}</span>
                            </Tooltip>
                          </TableCell>

                          <TableCell style={{ fontSize: '0.75rem' }}>{row.img ? 'Mevcut' : 'Mevcut Değil'}</TableCell>
                          <TableCell style={{ fontSize: '0.75rem' }}>
                            {row.icerik ? 'Mevcut' : 'Mevcut Değil'}
                          </TableCell>
                          <TableCell>
                            <Button
                              variant="contained"
      ...
      <Dialog open={open} onClose={handleClose} fullWidth maxWidth="sm">
        ...
        <DialogContent>
            ...
            {/* alttakı kod blogu silinmesi lazım canlıdan da... */}
