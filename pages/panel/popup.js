...
export default function Pupup() {
  ...
  const getData = async () => {
    ...
    setHasError(false);
    try {
      const response = await axios.get(API_ROUTES.PUPUP_PAGINATIONS.replace("currentPage", currentPage));
  ...
  const handleSave = (editedItem) => {
    ...
    axios.put(API_ROUTES.PUPUP_DETAIL.replace("id", editedItem.id), formData)
      .then(response => {
        ...
        return axios.get(API_ROUTES.PUPUP_PAGINATIONS.replace("currentPage", currentPage));
      })
      .then(response => {
        setData(response.data.results);
        handleClose();
  ...
  const handleConfirmDelete = () => {
    ...
    axios.post(API_ROUTES.PUPUP_DELETE, { ids: selectedIds })
      .then(() => {
        return axios.get(API_ROUTES.PUPUP);
      })
      .then((response) => {
        ...
        if (newTotalPages === 0) {
          ...
          return axios.get(API_ROUTES.PUPUP_PAGINATIONS.replace('currentPage', updatedPage));
        }
      })
      .then((response) => {
        if (response) {
