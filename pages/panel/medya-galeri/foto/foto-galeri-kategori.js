...
export default function FotoGaleriKategori() {
    ...
    const getData = async () => {
      ...
      setHasError(false);
      try {
        const response = await axios.get(API_ROUTES.FOTO_GALERI_KATEGORI_PAGINATIONS.replace("currentPage",currentPage))
    ...
    const handlePageChange = (event, value) => {
      ...
      const handleSave = (editedItem) => {
        ...
        setIsSaving(true);
        axios.put(API_ROUTES.FOTO_GALERI_KATEGORI_DETAIL.replace("id",editedItem.id), formData)
          .then(response => {
      ...
      const handleAddNewItem = () => {
        ...
        setIsSaving(true); 
        axios.post(API_ROUTES.FOTO_GALERI_KATEGORI, formData)
          .then(response => {
            // Mevcut sayfayı yeniden yüklüyoru
            return axios.get(API_ROUTES.FOTO_GALERI_KATEGORI_PAGINATIONS.replace("currentPage",currentPage))
          })
          .then(response => {
            setData(response.data.results);
            setTotalPages(Math.ceil(response.data.count / 10));
    ...
    const handleConfirmDelete = () => {
      setDeleteError('');
      axios.post(API_ROUTES.FOTO_GALERI_KATEGORI_DELETE, { ids: selectedIds })
        .then(() => {
          return axios.get(API_ROUTES.FOTO_GALERI_KATEGORI);
        })
        .then((response) => {
          ...
          if (newTotalPages === 0) {
            ...
            return axios.get(API_ROUTES.FOTO_GALERI_KATEGORI_PAGINATIONS.replace('currentPage', updatedPage));
          }
        })
        .then((response) => {
          if (response) {
