...
export default function VideoGaleriKategori() {
    ...
    const getData = async () => {
      ...
      setHasError(false);
      try {
        const response = await axios.get(API_ROUTES.VIDEO_GALERI_KATEGORI_PAGINATIONS.replace("currentPage",currentPage))
      ...
      const handleAddNewItem = () => {
        ...
        axios.post(API_ROUTES.VIDEO_GALERI_KATEGORI, formData)
          .then(response => {
            ...
            return axios.get(API_ROUTES.VIDEO_GALERI_KATEGORI_PAGINATIONS.replace("currentPage",currentPage))
          })
          .then(response => {
            setData(response.data.results);
            setTotalPages(Math.ceil(response.data.count / 10));
    ...
    const handleConfirmDelete = () => {
      ...
      axios.post(API_ROUTES.VIDEO_GALERI_KATEGORI_DELETE, { ids: selectedIds })
        .then(() => {
          return axios.get(API_ROUTES.VIDEO_GALERI_KATEGORI);
        })
        .then((response) => {
          ...
          if (newTotalPages === 0) {
            ...
            return axios.get(API_ROUTES.VIDEO_GALERI_KATEGORI_PAGINATIONS.replace('currentPage', updatedPage));
          }
        })
        .then((response) => {
          if (response) {
