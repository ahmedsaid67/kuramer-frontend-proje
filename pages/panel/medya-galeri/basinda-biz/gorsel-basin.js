...
export default function GorselBasin() {
    ...
    const getData = async () => {
      ...
      setHasError(false);
      try {
        const response = await axios.get(API_ROUTES.GORSEL_BASIN_PAGINATIONS.replace("currentPage",currentPage))
    ...
    const handlePageChange = (event, value) => {
      ...
      const handleAddNewItem = () => {
        ...
        if (!newItem.baslik || !newItem.kapakFotografi || !newItem.url ) {
        ...
        axios.post(API_ROUTES.GORSEL_BASIN, formData)
          .then(response => {
            ...
            return axios.get(API_ROUTES.GORSEL_BASIN_PAGINATIONS.replace("currentPage",currentPage))
          })
          .then(response => {
            setData(response.data.results);
            setTotalPages(Math.ceil(response.data.count / 10));
    ...
    const handleConfirmDelete = () => {
      ...
      axios.post(API_ROUTES.GORSEL_BASIN_DELETE, { ids: selectedIds })
        .then(() => {
          return axios.get(API_ROUTES.GORSEL_BASIN);
        })
        .then((response) => {
          ...
          if (newTotalPages === 0) {
            ...
            return axios.get(API_ROUTES.GORSEL_BASIN_PAGINATIONS.replace('currentPage', updatedPage));
          }
        })
        .then((response) => {
          if (response) {
