...
export default function KamuoyuDuyurulari() {
    ...
    const getData = async () => {
      ...
      setHasError(false);
      try {
        const response = await axios.get(API_ROUTES.KAMUOYU_DUYURULARI_PAGINATIONS.replace("currentPage",currentPage))
    ...
    const handlePageChange = (event, value) => {
      ...
      const handleSave = (editedItem) => {
        ...
        if (!editedItem.baslik || !editedItem.kapak_fotografi || !editedItem.tarih || !editedItem.pdf_dosya) {
        ...
        setIsSaving(true);
        axios.put(API_ROUTES.KAMUOYU_DUYURULARI_DETAIL.replace("slug",editedItem.slug), formData)
          .then(response => {
      ...
      const handleAddNewItem = () => {
        ...
        if (!newItem.baslik || !newItem.kapakFotografi || !newItem.tarih || !newItem.pdfDosya ) {
        ...
        setIsSaving(true); 
        axios.post(API_ROUTES.KAMUOYU_DUYURULARI, formData)
          .then(response => {
            // Mevcut sayfayı yeniden yüklüyoru
            return axios.get(API_ROUTES.KAMUOYU_DUYURULARI_PAGINATIONS.replace("currentPage",currentPage))
          })
          .then(response => {
            setData(response.data.results);
            setTotalPages(Math.ceil(response.data.count / 10));
    ...
    const handleConfirmDelete = () => {
      setDeleteError('');
      axios.post(API_ROUTES.KAMUOYU_DUYURULARI_DELETE, { ids: selectedIds })
        .then(() => {
          return axios.get(API_ROUTES.KAMUOYU_DUYURULARI);
        })
        .then((response) => {
          ...
          if (newTotalPages === 0) {
            ...
            return axios.get(API_ROUTES.KAMUOYU_DUYURULARI_PAGINATIONS.replace('currentPage', updatedPage));
          }
        })
        .then((response) => {
          if (response) {
    ...
    return(
      ...
      <DialogContent>
       ...
       <div style={{ textAlign: 'center', marginBottom: '20px', border: '2px dashed grey', padding: '10px', position: 'relative' }}> {/* position: relative eklendi */}
            {!newItem.kapakFotografi ? (
                <>
                    <Typography variant="subtitle1" style={{ marginBottom: '10px', position: 'absolute', top: 10, left: 10 }}>
                        Kapak Fotoğrafı:
                    </Typography>
                    <label htmlFor="kapak_fotografiInput">
                        <IconButton
                ...
                <>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative' }}> {/* İç div'e de position: relative eklendi */}
                        <img
