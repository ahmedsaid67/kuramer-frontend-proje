...
export default function TemelKonular() {
    ...
    const getData = async () => {
      ...
      setHasError(false);
      try {
        const response = await axios.get(API_ROUTES.TEMEL_KONULAR_PAGINATIONS.replace("currentPage",currentPage))
      ...
      const handleAddNewItem = () => {
        ...
        axios.post(API_ROUTES.TEMEL_KONULAR, formData)
          .then(response => {
            ...
            return axios.get(API_ROUTES.TEMEL_KONULAR_PAGINATIONS.replace("currentPage",currentPage))
          })
          .then(response => {
            setData(response.data.results);
            setTotalPages(Math.ceil(response.data.count / 10));
