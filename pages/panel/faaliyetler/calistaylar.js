  ...
  useEffect(() => {
      ...
      const handleAddNewItem = () => {
        ...
        axios.post(API_ROUTES.CALISTAYLAR, formData)
          .then(response => {
            ...
            return axios.get(API_ROUTES.CALISTAYLAR_PAGINATIONS.replace("currentPage",currentPage))
          })
          .then(response => {
            setData(response.data.results);
            setTotalPages(Math.ceil(response.data.count / 10));
    ...
    const handleConfirmDelete = () => {
      ...
      axios.post(API_ROUTES.CALISTAYLAR_DELETE, { ids: selectedIds })
        .then(() => {
          return axios.get(API_ROUTES.CALISTAYLAR);
        })
        .then((response) => {
          ...
          if (newTotalPages === 0) {
            ...
            return axios.get(API_ROUTES.CALISTAYLAR_PAGINATIONS.replace('currentPage', updatedPage));
          }
        })
        .then((response) => {
          if (response) {
