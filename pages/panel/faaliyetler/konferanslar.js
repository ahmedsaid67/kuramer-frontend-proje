...
export default function Konferanslar() {
  ...
  const getData = async () => {
    ...
    setHasError(false);
    try {
      const response = await axios.get(API_ROUTES.KONFERANSLAR_PAGINATIONS.replace("currentPage",currentPage))
      ...
      const handleAddNewItem = () => {
        ...
        axios.post(API_ROUTES.KONFERANSLAR, formData)
          .then(response => {
            ...
            return axios.get(API_ROUTES.KONFERANSLAR_PAGINATIONS.replace("currentPage",currentPage))
          })
          .then(response => {
            setData(response.data.results);
            setTotalPages(Math.ceil(response.data.count / 10));
    ...
    return(
      ...
      <Dialog open={open} onClose={handleClose} fullWidth maxWidth="sm">
        ...
        <DialogContent>
            ...
            <TextField
                label="Konuşmacı"
                value={selectedItem ? selectedItem.konusmaci : ''}
                onChange={(e) => setSelectedItem({ ...selectedItem, konusmaci: e.target.value })}
                fullWidth
                margin="normal"
            />
      ...
      <DialogContent>
        ...
        <TextField
          label="Konuşmacı"
          value={newItem.konusmaci}
          onChange={(e) => setNewItem({ ...newItem, konusmaci: e.target.value })}
          fullWidth
          margin="normal"
        />
