...
export default function Arastirmalar() {
  ...
  useEffect(() => {
    ...
    const getData = async () => {
      ...
      setHasError(false);
      try {
        const response = await axios.get(API_ROUTES.ARASTIRMALAR_PAGINATIONS.replace("currentPage",currentPage))
      ...
      const handleAddNewItem = () => {
        ...
        axios.post(API_ROUTES.ARASTIRMALAR, formData)
          .then(response => {
            ...
            return axios.get(API_ROUTES.ARASTIRMALAR_PAGINATIONS.replace("currentPage",currentPage))
          })
          .then(response => {
            setData(response.data.results);
            setTotalPages(Math.ceil(response.data.count / 10));
