...
function Personeller() {
      ...
      try {
        ...
        if (tabUrlFriendly && !isValidTab) {
          ...
          if (tabUrlFriendly) {
            ...
            const matchingCategory = categories.find(category => convertToUrlFriendly(category.name) === tabUrlFriendly);
            if (matchingCategory) {
              pagesFetchData()
              setDinamicPage({name: matchingCategory.name, url: path});
              
            }
          } else {
            // İlk kategoriye varsayılan olarak dön
            const initialTab = convertToUrlFriendly(categories[0]?.name);
            setActiveTab(initialTab);
            pagesFetchData()
            setDinamicPage({name:categories[0]?.name , url: `${path}?tab=${initialTab}`});
          }
        }
  ...
  const fetchBooks = async (kategoriId,page) => {
    ...
    try {
      ...
      //console.log("res:",kitapResponse.data.results)
  ...
  useEffect(() => {
    if (activeTab && kategoriler.length > 0) {
      ...
      if (selectedKategori) {
        ...
        setDinamicPage({ name: selectedKategori.name, url: newPath });
  ...
  useEffect(() => {
    if (router.query.tab && kategoriler.length > 0) {
      ...
      if (selectedKategori) {
        ...
        pagesFetchData()
        setDinamicPage({name:selectedKategori.name , url: `${path}`});
      }
    }else if (router && kategoriler.length > 0){
      const initialTab = convertToUrlFriendly(kategoriler[0]?.name);
      setActiveTab(initialTab);
  ...
  return (
    <>
      ...
      <div className={styles.mainContainer}>
          ...
          >
            {kategoriler.map(kategori => (
              <Tab  sx={{
                ...
                fontWeight: 'bold',
                textTransform: 'none',
              }}>{kategori.name}</Typography>} value={convertToUrlFriendly(kategori.name)} />
            ))}
          </Tabs>
        </div>
            ...
            {kategoriler.map(kategori => (
              <TabPanel key={kategori.id} value={activeTab} index={convertToUrlFriendly(kategori.name)}>
                ...
                <div className={styles.cardContainer}>
                    ...
                    <PersonellerCardOge key={index} yayin={yayin} path={path} activeTab={activeTab}/>
                    ))}
                </div> 
                ) : (
        ...
        <div className={styles.noDataMessage}>Kayıtlı Personel Kategorisi bulunmamaktadır.</div>)
      }
    </>
  );
}
